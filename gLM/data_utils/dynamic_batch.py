...
class LengthAdaptiveBatchSampler(Sampler):
    ...
    def __iter__(self):
        ...
        max_debug_batches = 0  # Number of batches to to print debug info for
        ...
        for i, batch_idx in enumerate(batch_order):
            if i < max_debug_batches:
                ...
                print(
                    f"[Rank {self.rank}] Yielding batch {i // self.world_size + 1}/{math.ceil(len(batched_indices) / self.world_size)} with {total_samples} samples, avg seq length: {avg_len:.2f}, min: {min_len}, max: {max_len}"
                )
